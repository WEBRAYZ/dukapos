...
interface PurchasesNavbarProps {
  activeTab: string;
  onTabChange: (tab: string) => void;
  slim?: boolean;
}

const PurchasesNavbar: React.FC<PurchasesNavbarProps> = ({ activeTab, onTabChange, slim }) => {
  const navItems = [
    { name: 'Purchase Orders', icon: '📋', count: 12 },
    { name: 'Receive Stock', icon: '📦', count: null },
    { name: 'Purchase History', icon: '🕓', count: null },
  ];
    ...
    <div className="hidden lg:flex items-center space-x-6 border-r border-blue-700/50 pr-4">
      <div className="flex flex-col items-end">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-[8px] font-black text-white/40 uppercase tracking-tighter mt-1">Orders</span>
        </div>
      </div>
      <div className="flex flex-col items-end">
        <div className="flex items-center space-x-2">
          <span className="text-xs font-black text-orange-400">5</span>
    ...
    >
      <div className="flex flex-col space-y-3 w-full">
        ...
        <div className="flex flex-col space-y-1 w-full">
          <p className="text-[8px] font-black text-blue-400 uppercase tracking-widest px-4 pb-1">Procurement</p>
          {navItems.map((item) => (
            <button
              key={item.name}
